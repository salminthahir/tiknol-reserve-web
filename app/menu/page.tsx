...
import { ShoppingBag, X, Plus, Minus, ArrowLeft, ChevronDown, Search } from 'lucide-react';
...
export default function MenuPage() {
  ...
  // LOGIC: Grouping Items by Category untuk tampilan
  const groupedMenu = {
  ...
  const handleCheckout = () => {
    ...
    window.open(`https://wa.me/6285349324001?text=${message}`, '_blank');
  };

  return (
    <div className="min-h-screen bg-[#1A1A1A] text-[#F5F5F5] font-sans selection:bg-[#FBC02D] selection:text-black pb-32">
          ...
          <div key={categoryName}>
            ...
            <div className="border-t border-[#FBC02D]/20">
              {items.map((item) => (
                <div 
                  key={item.id} 
                  className={`border-b border-[#FBC02D]/20 transition-all duration-300 ${expandedId === item.id ? 'bg-[#252525]' : 'hover:bg-[#252525]/50'}`}
                >
                  {/* BARIS UTAMA */}
                  <div 
                    onClick={() => toggleExpand(item.id)}
                    className="cursor-pointer py-6 px-2 md:px-6 flex flex-col md:flex-row md:items-center justify-between gap-4"
                  >
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                         <span className="text-[10px] font-bold text-gray-500 tracking-widest border border-gray-600 px-1">
                          {item.category}
                        </span>
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto mt-2 md:mt-0">
                      <span className="text-xl font-mono text-[#FBC02D]">
                        {item.price.toLocaleString('id-ID')}
                      </span>
                      <button 
                        onClick={(e) => addToCart(item, e)}
                        className="w-12 h-12 border border-[#FBC02D] bg-black flex items-center justify-center hover:bg-[#FBC02D] hover:text-black transition-colors z-10"
                      >
                        <Plus size={24} />
                      </button>
                    </div>
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-[60]">
          ...
          <div className="absolute right-0 top-0 h-full w-full max-w-md bg-[#1E1E1E] border-l border-[#FBC02D] p-6 flex flex-col shadow-2xl">
            ...
            <div className="pt-6 border-t border-[#FBC02D]">
              ...
              <button 
                ...
                className="w-full bg-[#E64A19] py-4 font-bold tracking-widest uppercase hover:bg-white hover:text-black transition-colors disabled:bg-gray-700"
              >
                PROCEED TO WHATSAPP
              </button>
            </div>
